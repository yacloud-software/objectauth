...
Alter statements:
ALTER TABLE grouptoobject ADD COLUMN objecttype integer not null default 0;
ALTER TABLE grouptoobject ADD COLUMN objectid bigint not null default 0;
ALTER TABLE grouptoobject ADD COLUMN groupid text not null default '';
ALTER TABLE grouptoobject ADD COLUMN active boolean not null default false;
ALTER TABLE grouptoobject ADD COLUMN read boolean not null default false;
ALTER TABLE grouptoobject ADD COLUMN write boolean not null default false;
ALTER TABLE grouptoobject ADD COLUMN execute boolean not null default false;
ALTER TABLE grouptoobject ADD COLUMN view boolean not null default false;
...
func (a *DBGroupToObject) ByID(ctx context.Context, p uint64) (*savepb.GroupToObject, error) {
	...
	if len(l) == 0 {
		return nil, a.Error(ctx, qn, fmt.Errorf("No GroupToObject with id %d", p))
	}
	if len(l) != 1 {
		return nil, a.Error(ctx, qn, fmt.Errorf("Multiple (%d) GroupToObject with id %d", len(l), p))
	}
	return l[0], nil
}
